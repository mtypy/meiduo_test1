...
def merge_cookie_cart_to_redis(request, user, response):
    ...
    # 保存cookie购物车中未被勾选的商品的id，此列表中的元素在进行购物车记录合并时需要从redis set中移除
    ...
    # 将`cart`字典中的key和value作为属性和值设置到redis的hash元素中，如果已经存在的属性和值会进行覆盖
    ...
    if cart_selected_add:
        # 将`cart_selected_add`列表中商品的id添加到redis set中
        redis_conn.sadd(cart_selected_key, *cart_selected_add)

    if cart_selected_remove:
        # 将`cart_selected_remove`列表中商品的id从redis set中移除
